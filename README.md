...
**次のマイルストーン**: テーマ自動取得とPrompt Caching最適化

## 🔧 最近の修正（2026-02-07）

### Prompt Caching実装（75%割引）✅
- **gpt-4.1-nano使用**: OpenAI最新の最速・最安モデル（2025-04-14リリース）
- **cache_control設定**:
  - システムプロンプト: `cache_control: { type: 'ephemeral' }`
  - 会話履歴の最後のメッセージ: `cache_control: { type: 'ephemeral' }`
  - 新しいユーザー指示: キャッシュ対象外（毎回変わる）
- **コスト削減効果**:
  - キャッシュヒット: 75%割引（$0.11/1M → $0.03/1M）
  - 10ターン想定: 約70%がキャッシュヒット
  - **推定コスト**: 約$0.007/ディベート（約¥1/ディベート）
- **トークンログ追加**: `console.log`でInput/Output/Cache Readを出力

### テーマごとのプロンプトテンプレート化✅
- **現在の実装**: 
  - テーマ（DEBATE_THEME）、立場A（OPINION_A）、立場B（OPINION_B）を使用
  - HTMLから読み込み（今後D1から自動取得予定）
- **プロンプト構造**:
  ```
  【テーマ】${DEBATE_THEME}
  【あなたの立場】${OPINION_A / OPINION_B}
  【厳守事項】1-5項目
  【禁止事項】具体例
  ```
- **完全汎用化**: テーマに依存しない汎用プロンプト設計

### プロンプト品質の改善✅
- **重複プロンプト削除**: 旧バージョンの残骸を削除
- **禁止事項の明確化**: 
  - 相手の論点を認める表現（「確かに」「一方で」など）
  - 抽象的で曖昧な表現（「可能性がある」「考えられる」など）
  - 架空の統計や数値
- **具体性の強化**: データ・事例・統計の使用を必須化

### 会話履歴送信の最適化✅
- **全履歴を送信**: `conversationHistory.slice(-3)` → `conversationHistory`（全履歴）
- **Prompt Caching活用**: 履歴部分はキャッシュ、最後の指示のみ非キャッシュ
- **インプット量の適正化**: 1ターン目100トークン → 10ターン目約1,500トークン（適正範囲）

### 文字制限の統一（180文字）✅
- **フロントエンド**: 180文字プロンプト
- **バックエンド**: 180文字強制切り詰め
- **max_tokens**: 220トークン（180文字相当）
...
1. **ディベート生成（2AI）**
   - 新規インプット: 約4,500トークン
   - キャッシュヒット: 約11,000トークン
   - アウトプット: 約3,000トークン
   - コスト: (4,500 × 0.11/1M) + (11,000 × 0.03/1M) + (3,000 × 0.44/1M) = **$0.002145**

2. **AI評価（3ターン目以降、8ターン分、16回）**
   - 新規インプット: 約3,200トークン
   - キャッシュヒット: 約9,600トークン
   - アウトプット: 約1,280トークン
   - コスト: (3,200 × 0.11/1M) + (9,600 × 0.03/1M) + (1,280 × 0.44/1M) = **$0.001275**

3. **AI審査員（3ターン目以降、3人、24回）**
   - 新規インプット: 約3,600トークン
   - キャッシュヒット: 約10,800トークン
   - アウトプット: 約1,200トークン
   - コスト: (3,600 × 0.11/1M) + (10,800 × 0.03/1M) + (1,200 × 0.44/1M) = **$0.001452**

**1回のディベート合計コスト**: **$0.004872**
- 約 **$0.005 / ディベート**（10ターン）
- 約 **¥0.75 / ディベート**（1ドル=150円換算）

**キャッシュなし（修正前）との比較**
- 修正前: $0.009693（キャッシュ0）
- 修正後: $0.004872（キャッシュ75%割引）
- **削減率: 約50%**
